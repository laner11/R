...
#读取ARCGIS源文件
file_con <- file("Data/test/南京市0811_FeaturesToJSON.json", "r")
...
#清洗图层暂存
write(data_t1,"Data/北京2_arcgis.txt")

#读入清洗后数据
data00<-read.table("Data/北京2_arcgis.txt",header = FALSE, sep = "\t",quote = "") 
#test
t_str<-data00$V1[1]
...
tt_sub0<-substr(tt,1,idCnter[1]-2)
...
s_result<-paste('"ID":',s_ID,",",'"name":',paste('"',s_name,"_",s_ID,'"'),",",'"center":',"\\[",tt_sub1,"\\]")
...
out#输出帆软格式的json
write(out_str,"Data/深圳市-area.json")
